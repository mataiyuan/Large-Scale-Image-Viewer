...
namespace VirtualCanvasLib
{
    ...
    internal class RectangleSelectionGesture
    {
        ...
        Rect _selectionRect;
        bool _fszoomSelection;
        int _fszoomSizeThreshold = 20;
        ...
        {
            get { return _fszoomSelection; }
            set { _fszoomSelection = value; }
        }            
        ...
        {
            get { return _fszoomSizeThreshold; }
            set { _fszoomSizeThreshold = value; }
        }
        ...
        void OnMouseMove(object sender, MouseEventArgs e)
        {
            if (_watching)
            {
                ...
                if (new Vector(_start.X - pos.X, _start.Y - pos.Y).Length > _selectionThreshold)
                {
                    ...
                    _selectionRectVisual = new SelectionRectVisual(_start, _start, _fszoom.Zoom);
                    _container.Children.Add(_selectionRectVisual);
                }
            }
            if (_selectionRectVisual != null)
            {
                if (_selectionRectVisual.Zoom != _fszoom.Zoom)
                {
                    _selectionRectVisual.Zoom = _fszoom.Zoom;
                }
        ...
        void OnMouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            ...
            if (_selectionRectVisual != null)
            {
                ...
                // if (_fszoomSelection && f > _fszoomSizeThreshold)
                // 限制缩放比例
                if (_fszoomSelection && f > _fszoomSizeThreshold && _fszoom.Zoom <=1 )
                {
                    _fszoom.ZoomToRect(r);
                }
